  ...
  gpsx__srv__GetSatList_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    gpsx__srv__GetSatList_Request * data =
      (gpsx__srv__GetSatList_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gpsx__srv__GetSatList_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gpsx__srv__GetSatList_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  gpsx__srv__GetSatList_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    gpsx__srv__GetSatList_Response * data =
      (gpsx__srv__GetSatList_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gpsx__srv__GetSatList_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gpsx__srv__GetSatList_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
